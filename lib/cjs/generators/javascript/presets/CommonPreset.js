...
function renderMarshalProperty(modelInstanceVariable, model, inputModel) {
    if (model.$ref) {
...
function renderMarshalProperties(model, renderer, inputModel) {
...
function renderMarshalPatternProperties(model, renderer, inputModel) {
...
function renderMarshalAdditionalProperties(model, renderer, inputModel) {
...
function renderUnmarshalProperty(modelInstanceVariable, model, inputModel, renderer) {
    if (model.$ref) {
        const resolvedModel = inputModel.models[model.$ref];
        const propertyModelKind = helpers_1.TypeHelpers.extractKind(resolvedModel);
        //Referenced enums only need standard marshalling, so lets filter those away
...
function renderUnmarshalProperties(model, renderer, inputModel) {
    const properties = model.properties || {};
    const propertyKeys = [...Object.entries(properties)];
...
function renderUnmarshalPatternProperties(model, renderer, inputModel) {
...
function renderUnmarshalAdditionalProperties(model, renderer, inputModel) {
    ...
    class: {
        additionalContent({ renderer, model, content, options, inputModel }) {
            ...
            if (options.example === true) {
                blocks.push((0, ExampleFunction_1.default)({ renderer, model }));
            }
