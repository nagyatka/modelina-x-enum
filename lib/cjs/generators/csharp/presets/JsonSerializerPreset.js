...
function renderSerializeProperty(modelInstanceVariable, model, inputModel) {
    let value = modelInstanceVariable;
    if (model.$ref) {
        const resolvedModel = inputModel.models[model.$ref];
        const propertyModelKind = helpers_1.TypeHelpers.extractKind(resolvedModel);
        //Referenced enums is the only one who need custom serialization
        if (propertyModelKind === helpers_1.ModelKind.ENUM) {
            value = `${value}.GetValue()`;
        }
    }
    return `JsonSerializer.Serialize(writer, ${value}, options);`;
}
function renderSerializeAdditionalProperties(model, renderer, inputModel) {
    const serializeAdditionalProperties = '';
    if (model.additionalProperties !== undefined) {
        let additionalPropertyName = (0, helpers_1.getUniquePropertyName)(model, helpers_1.DefaultPropertyNames.additionalProperties);
        additionalPropertyName = helpers_1.FormatHelpers.upperFirst(renderer.nameProperty(additionalPropertyName, model.additionalProperties));
...
function renderSerializeProperties(model, renderer, inputModel) {
    let serializeProperties = '';
    if (model.properties !== undefined) {
        for (const [propertyName, propertyModel] of Object.entries(model.properties)) {
            const formattedPropertyName = helpers_1.FormatHelpers.upperFirst(renderer.nameProperty(propertyName, propertyModel));
...
function renderSerializePatternProperties(model, renderer, inputModel) {
    ...
    if (model.patternProperties !== undefined) {
        for (const [pattern, patternModel] of Object.entries(model.patternProperties)) {
            let patternPropertyName = (0, helpers_1.getUniquePropertyName)(model, `${pattern}${helpers_1.DefaultPropertyNames.patternProperties}`);
            patternPropertyName = helpers_1.FormatHelpers.upperFirst(renderer.nameProperty(patternPropertyName, patternModel));
...
function renderDeserializeProperty(type, model, inputModel) {
    if (model.$ref) {
        const resolvedModel = inputModel.models[model.$ref];
        const propertyModelKind = helpers_1.TypeHelpers.extractKind(resolvedModel);
        //Referenced enums is the only one who need custom serialization
...
function renderDeserializeProperties(model, renderer, inputModel) {
    const propertyEntries = Object.entries(model.properties || {});
    const deserializeProperties = propertyEntries.map(([prop, propModel]) => {
        const formattedPropertyName = helpers_1.FormatHelpers.upperFirst(renderer.nameProperty(prop, propModel));
...
function renderDeserializePatternProperties(model, renderer, inputModel) {
    if (model.patternProperties === undefined) {
        return '';
    }
...
function renderDeserializeAdditionalProperties(model, renderer, inputModel) {
    if (model.additionalProperties === undefined) {
        return '';
    }
    let additionalPropertyName = (0, helpers_1.getUniquePropertyName)(model, helpers_1.DefaultPropertyNames.additionalProperties);
