...
function renderMarshalProperties(renderer, model) {
    const properties = model.properties || {};
    const propertyKeys = [...Object.keys(properties)];
...
function renderUnmarshalProperties(renderer, model) {
    const properties = model.properties || {};
    const propertyKeys = [...Object.keys(properties)];
    const unmarshalProperties = propertyKeys.map(prop => {
        ...
        if (propModel.type === 'undefined') {
            LoggingInterface_1.Logger.error(`Could not render unmarshal for property ${prop}`);
            return;
        }
        ...
        const getType = `jsonObject.get${helpers_1.FormatHelpers.upperFirst(((_a = propModel.type) === null || _a === void 0 ? void 0 : _a.toString()) || '')}`;
        return `if(jsonObject.has("${formattedPropertyName}")) {
